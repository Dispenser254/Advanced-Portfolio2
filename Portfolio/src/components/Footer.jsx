                ...
                <div key={id} className="group w-fit px-3">
                    <Link to={href} smooth duration={500}>
                        <div className="flex items-center justify-between w-fit gap-2 font-semibold text-white cursor-pointer">
                            <FaArrowRight />
                            <span className="bg-clip-text bg-gradient-to-r from-cyan-500 to-blue-500 group-hover:text-transparent duration-500 group-hover:animate-pulse">{href}</span>
                        </div>
                    </Link>
                </div>
            ))}
            ...
            <div className="flex flex-col sm:flex-row lg:flex-col gap-5 xs:gap-10 sm:gap-5 md:gap-10 justify-between">
              ...
              <ul className="flex items-center gap-5 md:gap-8 lg:gap-4 justify-start">
                {contactInfo.map(({id, link, title, icon, download}) => (
                    <li key={id} className='group relative w-8 h-8 flex justify-center items-center rounded-full p-1 sm:grayscale cursor-pointer duration-[450ms] transition ease-in hover:grayscale-0 focus:scale-105 active:grayscale-0 active:scale-105'>
                        <a href={link} download={download} target="_blank" rel="noopener noreferrer">{icon}</a>
                        <div className="flex scale-0 sm:group-hover:scale-100 group-focus:scale-100 transition ease-out duration-300 delay-100 origin-center justify-center items-center absolute -bottom-10 w-auto h-6 py-2 px-3 bg-gray-900 text-sm text-gray-300">{title}</div>
                    </li>
                ))}
              </ul>
            </div>
      ...
      <div className="section py-2 text-black font-advanced">
        <div>
            <p className="text-sm text-gray-300 flex justify-center items-center">
                <span className="px-1 pb-[0.2rem]">&copy;</span>{year} frodenZlabs.com| All rights reserved
            </p>
        </div>
      </div>
    </div>
